...
$sql_revenue = "SELECT SUM(final_amount) as total FROM orders WHERE order_status = 'delivered' AND payment_status = 'paid'";
...
// Today's Revenue
$sql_today_rev = "SELECT SUM(final_amount) as total FROM orders 
...
// This Month Revenue
$sql_month_rev = "SELECT SUM(final_amount) as total FROM orders 
...
$sql_last_month_rev = "SELECT SUM(final_amount) as total FROM orders 
...
// Today's Orders
$sql_today_orders = "SELECT COUNT(*) as count FROM orders WHERE DATE(created_at) = '$today'";
...
$sql_last_month_orders = "SELECT COUNT(*) as count FROM orders 
                          WHERE DATE(created_at) >= '$last_month_start' 
                          AND DATE(created_at) <= '$last_month_end'";
...
if ($last_month_orders > 0) {
  ...
  $order_growth = 100; // First month with orders
}

// Total Customers (only customers, not all users)
$sql_customers = "SELECT COUNT(*) as count FROM users WHERE user_type = 'customer'";
...
$sql_last_month_customers = "SELECT COUNT(*) as count FROM users 
                             ...
                             AND DATE(created_at) >= '$last_month_start' 
                             AND DATE(created_at) <= '$last_month_end'";
...
} elseif ($total_customers > 0) {
  $customer_growth = 100; // First month with customers
}
...
// Last Month Products (new products added last month)
...
if ($last_month_products > 0) {
  ...
  $product_growth = 100; // First month with products
}
...
for ($i = 29; $i >= 0; $i--) {
  ...
  $sql_sales = "SELECT SUM(final_amount) as sales, COUNT(*) as orders 
                FROM orders 
...
// Get top products by sales (using order_items table)
$sql_top_products = "SELECT 
                     ...
                     AND p.status = 1
                     GROUP BY p.id, p.pro_id, p.pro_name
...
// Additional metrics for e-commerce dashboard

// Average Order Value
$sql_avg_order = "SELECT AVG(final_amount) as avg_value FROM orders WHERE order_status = 'completed'";
$res_avg_order = mysqli_query($conn, $sql_avg_order);
$avg_order_value = 0;
if ($res_avg_order && $row = mysqli_fetch_assoc($res_avg_order)) {
  $avg_order_value = $row['avg_value'] ? round($row['avg_value'], 2) : 0;
}
...
$sql_low_stock = "SELECT COUNT(*) as low_stock_count FROM products WHERE stock < 10 AND status = 1";
$res_low_stock = mysqli_query($conn, $sql_low_stock);
$low_stock_count = 0;
if ($res_low_stock && $row = mysqli_fetch_assoc($res_low_stock)) {
  $low_stock_count = $row['low_stock_count'];
}

// Get abandoned carts (carts older than 1 hour)
$sql_abandoned_carts = "SELECT 
                        COUNT(DISTINCT visitor_id) as abandoned_count,
                        SUM(p.selling_price * c.quantity) as abandoned_value
                        FROM cart c
                        JOIN products p ON c.product_id = p.id
                        WHERE c.created_at < DATE_SUB(NOW(), INTERVAL 1 HOUR)";
$res_abandoned_carts = mysqli_query($conn, $sql_abandoned_carts);
$abandoned_data = ['abandoned_count' => 0, 'abandoned_value' => 0];
if ($res_abandoned_carts && $row = mysqli_fetch_assoc($res_abandoned_carts)) {
  $abandoned_data = $row;
}

// Get today's new customers
$sql_today_customers = "SELECT COUNT(*) as count FROM users 
                        WHERE user_type = 'customer' 
                        AND DATE(created_at) = '$today'";
$res_today_customers = mysqli_query($conn, $sql_today_customers);
$today_customers = 0;
if ($res_today_customers && $row = mysqli_fetch_assoc($res_today_customers)) {
  $today_customers = $row['count'];
}
?>
                              ...
                              <?php if (!empty($recent_orders)): ?>
                                <?php foreach ($recent_orders as $order): ?>
                                  <tr>
                                    ...
                                    <td>
                                      <div class="text-truncate" style="max-width: 150px;" title="<?= htmlspecialchars($order['product_names']) ?>">
                                        <?= htmlspecialchars($order['product_names']) ?: 'No products' ?>
                                      </div>
                                    </td>
                                    <td class="fw-bold">
                      ...
                      <div class="card shadow-sm mb-4">
                        ...
                        <div class="card-body">
                          <div class="row g-3 text-center">
                            ...
                            <div class="col-6">
                              <a href="add-products.php"
                                class="d-block h-100 p-3 rounded bg-light text-decoration-none text-dark shadow-sm">
                                <div class="mb-2">
                         ...
                         bg-primary text-white rounded-circle"
                                ...
                                <h6 class="fw-semibold mb-1">Add Product</h6>
                         ...
                         bg-success text-white rounded-circle"
                                ...
                                <h6 class="fw-semibold mb-1">Manage Orders</h6>
                         ...
                         bg-warning text-white rounded-circle"
                                ...
                                <h6 class="fw-semibold mb-1">Products</h6>
  ...
  <script>
    ...
    var orderStatusChart = new ApexCharts(
      document.querySelector("#orderStatusChart"), {
        ...
        labels: ['Confirmed', 'Processing', 'Delivered', 'Cancelled'],
        colors: ['#f6c23e', '#4e73df', '#1cc88a', '#e74a3b'],
        legend: {
          position: 'bottom'
        },
        plotOptions: {
            ...
            donut: {
              ...
              labels: {
                ...
                total: {
                  ...
                  label: 'Total Orders',
                  formatter: function() {
                    return <?= (int)$status_stats['total_orders'] ?>;
                  }
                }
              }
            }
