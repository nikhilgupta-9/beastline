...
<body class="crm_body_bg">
    ...
    // ✅ ADD FLAGS
    formData.append('update_razorpay', '1');
    formData.append('update_smtp', '1');

    // Append Razorpay form
    const razorpayForm = document.getElementById('razorpayForm');
    if (razorpayForm) {
        new FormData(razorpayForm).forEach((value, key) => {
    ...
    if (smtpForm) {
        new FormData(smtpForm).forEach((value, key) => {
            formData.append(key, value);
        });
    }

    fetch('<?= ADMIN_URL ?>ajax/payment-smtp-setting.php', {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
