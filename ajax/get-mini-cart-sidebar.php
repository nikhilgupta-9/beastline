...
if (isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
    foreach ($_SESSION['cart'] as $key => $item) {
        ...
        $sql = "SELECT p.pro_id, p.pro_name, p.pro_img, p.selling_price,
                       pv.price as variant_price, pv.color, pv.size, pv.image as variant_img
                FROM products p
...
if ($item_count > 0) {
    ...
    foreach ($cart_items as $item) {
        ...
        $content .= '<img src="' . $site . 'admin/assets/img/uploads/variants/' . $item['image'] . '" alt="' . htmlspecialchars($item['name']) . '">';
        $content .= '</a>';
        $content .= '</div>';
    ...
    $content .= '<a href="' . $site . 'checkout/" class="btn btn-dark mt-2">Continue Shopping</a>';
    $content .= '</div>';
}
